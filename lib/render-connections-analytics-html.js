...
<body>
  ...
  <div class="wrap">
    ...
    String(analytics.summary.total)
          )} contacts analysed · ${escapeHtml(
    String(analytics.summary.disconnected || 0)
  )} disconnected</div>
      </div>
    ...
    <section class="stats fade-in">
      <div class="card stat-card"><div class="label">Still connected</div><div class="value" id="stat-total">${analytics.summary.connected ?? analytics.summary.total}</div><div class="hint">${analytics.summary.total} in the export</div></div>
      <div class="card stat-card"><div class="label">Disconnected</div><div class="value">${analytics.summary.disconnected || 0}</div><div class="hint">Removed in prune runs</div></div>
    ...
    <section class="card table-card fade-in">
      ...
      <p class="desc">Search and filter your full exported network, including people already disconnected.</p>
      <div class="toolbar">
        <input id="searchInput" type="search" placeholder="Search name, title, company..." />
        <select id="statusFilter">
          <option value="">All statuses</option>
          <option value="connected">Still connected</option>
  ...
  <script>
    ...
    function renderTable() {
      ...
      const rows = DATA.contacts.filter((item) => {
        ...
        if (status === 'disconnected' && !item.disconnected) return false;
        if (status === 'connected' && item.disconnected) return false;
        if (!q) return true;
        const haystack = [item.name, item.title, item.companies.join(' '), item.role, item.seniority, item.disconnected ? 'disconnected' : 'connected'].join(' ').toLowerCase();
        return haystack.includes(q);
      });

      contactsBody.innerHTML = rows.slice(0, 500).map((item) => \`
        <tr class="\${item.disconnected ? 'disconnected' : ''}">
          <td>\${item.name}</td>
          <td class="title-cell">\${item.title || '—'}</td>
          <td>\${item.role}</td>
          <td>\${item.seniority}</td>
          <td>\${item.connectedOn || '—'}</td>
          <td><span class="status-pill \${item.disconnected ? 'gone' : 'connected'}">\${item.disconnected ? 'Disconnected' : 'Connected'}</span></td>
          <td><a href="\${item.profileUrl}" target="_blank" rel="noopener">Open</a></td>
        </tr>\`).join('');

      if (rows.length > 500) {
        contactsBody.insertAdjacentHTML('beforeend', \`<tr><td colspan="7">Showing first 500 of \${rows.length} matches. Refine your search to narrow results.</td></tr>\`);
      }
    }
